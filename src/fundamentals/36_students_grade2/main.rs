...
You need to extend the Student struct by implementing two methods:
    ...
    add_grade: Add a grade to the student’s grades vector.
    average_grade(&self) -> f64: Calculate and return the average grade of the student. Return 0.0 if the student has no grades.
...
impl Student {
    ...
    pub fn average_grade(&self) -> f64 {
        ...
        let sum: u32 = self.grades.iter().map(|&grade| grade as u32).sum();
